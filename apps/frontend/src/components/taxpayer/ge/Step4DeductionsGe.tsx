...
// GE 2026 : plafond salarié 7'260 CHF, indépendant 36'288 CHF
const GE_PILIER_3A_SALARIE = 7260;
const GE_PILIER_3A_INDEPENDANT = 36288;
...
export function Step4DeductionsGe({ draft, year }: Props) {
  const update = useFieldUpdaterGe(year);
  const s = draft.state.step4;
  const isSalarie = draft.state.step2.isSalarie ?? false;
  const pilier3aMax = isSalarie ? GE_PILIER_3A_SALARIE : GE_PILIER_3A_INDEPENDANT;
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted">
          Déductions admises LIFD art. 33 et LIPP (RSG D 3 08). Le plafond pilier 3a dépend
        ...
        <div className="card p-4">
          ...
          <div className="flex gap-3">
            <label className="flex items-center gap-2 text-sm cursor-pointer">
              ...
              />
              Forfait (3% du salaire, min 1'700 / max 4'000 CHF)
            </label>
            <label className="flex items-center gap-2 text-sm cursor-pointer">
              <input
                type="radio"
