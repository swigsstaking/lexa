/**
 * PmWizardCanton — Wizard déclaration PM (Personnes Morales) générique 4 cantons
 * Session 28 — généralise PmWizardVs pour GE, VD, FR
 *
 * Props :
 *   canton: 'VS' | 'GE' | 'VD' | 'FR'
 *
 * Réutilise tous les Step components de vs/ (95% canton-agnostic pour PM).
 * Seuls le label header, la commune (via prop) et l'endpoint submit changent.
 */
...
export function PmWizardCanton({ canton }: Props) {
  ...
  async function loadOrCreateDraft() {
    ...
    try {
      ...
      setDraft(d);
      // BUG-8 fix : si le draft step2 est entièrement vide (nouveau draft),
      // pré-remplir depuis l'income statement du grand livre
      ...
      if (isStep2Empty) {
        try {
          ...
          // revenuesTotal → chiffreAffaires, chargesTotal → total charges, netResult → bénéfice
          if (is.revenuesTotal || is.chargesTotal || is.netResult) {
            ...
            // Appliquer les patches (debounced via handlePatch n'est pas disponible ici,
            // donc on les envoie directement et on met à jour le state local)
            await Promise.all(patches.map(([path, value]) =>
              lexa.patchCompanyDraft(year, canton, path, value).catch(() => {})
            ));
            // Recharger le draft avec les valeurs pré-remplies
            const dRefreshed = await lexa.getCompanyDraft(year, canton);
            setDraft(dRefreshed);
          }
        } catch {
          // Soft fail : si ledger vide ou indisponible, laisser le draft vide
        }
      }
    } catch {
      try {
        ...
        const d = await lexa.getCompanyDraft(year, canton);
        setDraft(d);
        // Même logique de pré-remplissage pour un nouveau draft
        try {
          const is = await lexa.getIncomeStatement(year);
          if (is.revenuesTotal || is.chargesTotal || is.netResult) {
            const patches: Array<[string, number]> = [];
      ...
      <main className="flex-1 overflow-auto">
        <div className="max-w-6xl mx-auto px-6 py-8 pb-48 lg:pb-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <aside className="lg:col-span-1 fixed bottom-0 left-0 right-0 lg:relative lg:bottom-auto bg-surface/95 lg:bg-transparent backdrop-blur-sm lg:backdrop-blur-none border-t lg:border-t-0 border-border p-4 lg:p-0 max-h-[40vh] lg:max-h-none overflow-y-auto z-10 lg:z-auto">
            <PmWizardSummaryVs draft={draft} canton={canton} />
          </aside>
        </div>
      </main>
