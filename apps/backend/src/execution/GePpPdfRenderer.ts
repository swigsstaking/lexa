...
export async function renderGePpPdf(form: FilledVsPpForm): Promise<Buffer> {
  const doc = new PDFDocument({
    size: "A4",
    margins: { top: 48, bottom: 48, left: 48, right: 48 },
    info: {
      Title: `Déclaration PP GE ${form.year} — ${form.company.name}`,
      Author: "Lexa",
      Subject: "Déclaration fiscale PP Canton de Genève",
    },
  });
  ...
  // ── Header ─────────────────────────────────────────
  doc.fontSize(18).fillColor("#0b0b0f").text("Déclaration d'impôt PP Genève — 2026", {
    align: "left",
  });
  doc.fontSize(10).fillColor("#71717a").text("Canton de Genève — Administration fiscale cantonale (AFC-GE)");
  doc
    .fontSize(9)
    .text(
      `LIFD art. 33, LIPP (RSG D 3 08) — art. ${form.template.legal_reference.articles.join(", ")}`,
    );
  doc.moveDown(1);
  ...
  doc.text(`Canton : ${form.company.canton ?? "GE"}`);
