...
export async function renderJuPpPdf(form: FilledVsPpForm): Promise<Buffer> {
  const doc = new PDFDocument({
    size: "A4",
    margins: { top: 48, bottom: 48, left: 48, right: 48 },
    info: {
      Title: `Déclaration PP JU ${form.year} — ${form.company.name}`,
      Author: "Lexa",
      Subject: "Déclaration fiscale PP Canton du Jura",
    },
  });
  ...
  doc.fontSize(18).fillColor("#0b0b0f").text("Déclaration d'impôt PP Jura — 2026", {
    align: "left",
  });
  doc.fontSize(10).fillColor("#71717a").text("Canton du Jura — Service cantonal des contributions du Jura (SCCJ)");
  doc
    .fontSize(9)
    .text(
      `LICD-JU (RSJU 641.11), LIFD — art. ${form.template.legal_reference.articles.join(", ")}`,
    );
  doc.moveDown(1);
  ...
  doc.text(`Canton : ${form.company.canton ?? "JU"}`);
  ...
  doc
    ...
    .fillColor("#991b1b")
    .text(
      "Information à titre indicatif — vérifiez avec votre fiduciaire et le Service cantonal des contributions du Jura (SCCJ).",
      48,
      760,
