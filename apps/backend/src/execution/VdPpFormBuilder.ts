...
// Forfait frais professionnels VD : min 2'000 CHF, max 4'000 CHF (3%)
// TODO session 20 : confirmer barème ACI VD Art. 26 LI
const VD_FRAIS_PRO_FORFAIT_MIN = 2000;
const VD_FRAIS_PRO_FORFAIT_MAX = 4000;
const VD_FRAIS_PRO_FORFAIT_PCT = 3;

// Coefficient communal par défaut : Lausanne 2026 = 79
// TODO à valider barème 2026 (source : vd.ch ou lausanne.ch/impots)
const VD_COEFFICIENT_COMMUNAL_LAUSANNE = 79;

// Exporté pour usage informatif uniquement
export const VD_CONSTANTS = {
  VD_PILIER_3A_WITH_LPP,
  VD_PILIER_3A_WITHOUT_LPP,
  VD_FRAIS_PRO_FORFAIT_MIN,
  VD_FRAIS_PRO_FORFAIT_MAX,
  VD_FRAIS_PRO_FORFAIT_PCT,
  VD_COEFFICIENT_COMMUNAL_LAUSANNE,
} as const;

const TEMPLATE_FILE = "vd-declaration-pp-2026.yaml";
const __dirname = dirname(fileURLToPath(import.meta.url));

let cachedTemplate: VsPpFormTemplate | null = null;

async function loadVdPpTemplate(): Promise<VsPpFormTemplate> {
...
/**
 * Projection fiscale PP Vaud avec support dual mode.
 *
 * Clone de GePpFormBuilder — adapté aux spécificités vaudoises :
 * - Forfait frais pro min = 2'000 CHF, max = 4'000 CHF (3%)
 *   TODO session 20 : confirmer barème ACI VD Art. 26 LI
 * - Pilier 3a salarié max = 7'260 CHF (avec LPP), 36'288 CHF (sans LPP) — 2026
 * - Coefficient communal au lieu de centimes additionnels GE
 * - PAS de forfait frontalier (VD n'a pas de régime frontalier significatif)
 * - Template YAML : vd-declaration-pp-2026.yaml
 * - Disclaimer : LI (BLV 642.11), LIPC (BLV 650.11), LIFD
 * - Autorité : ACI VD (Administration cantonale des impôts Vaud)
 * - Délai de dépôt : 15 mars
 */
...
/**
 * Calcul forfait frais professionnels VD.
 * Taux : 3%, min 2'000 CHF, max 4'000 CHF.
 * TODO session 20 : confirmer barème ACI VD Art. 26 LI
 */
function computeFraisProVd(
  salaireBrut: number,
  format?: "forfait" | "reel",
  fraisReels?: number,
): number {
  ...
  // TODO session 20 : confirmer barème ACI VD Art. 26 LI
  const rawForfait = base * (VD_FRAIS_PRO_FORFAIT_PCT / 100);
  return round2(
    Math.min(
      Math.max(rawForfait, VD_FRAIS_PRO_FORFAIT_MIN),
      VD_FRAIS_PRO_FORFAIT_MAX,
    ),
  );
}

async function projectVdPp(params: {
  tenantId: string;
  year: number;
  ...
  const { tenantId, year, draft, coefficientCommunal: _coefficientCommunal } = params;
  const hasDraft =
    !!draft &&
  ...
  // VD : frais pro calculés sur salaire + revenus accessoires
  // TODO session 20 : confirmer barème ACI VD Art. 26 LI
  const deductionFraisPro = computeFraisProVd(
...
export async function buildVdPpDeclaration(params: {
  tenantId: string;
  year: number;
  draft?: TaxpayerDraftState;
  coefficientCommunal?: number;
}): Promise<FilledVsPpForm> {
  const { tenantId, year, draft, coefficientCommunal } = params;
  const template = await loadVdPpTemplate();
  const company: CompanyInfo = await getCompany(tenantId);
  const projection = await projectVdPp({
    tenantId,
    year,
    refAmounts: template.reference_amounts,
    draft,
    coefficientCommunal: coefficientCommunal ?? VD_COEFFICIENT_COMMUNAL_LAUSANNE,
  });
