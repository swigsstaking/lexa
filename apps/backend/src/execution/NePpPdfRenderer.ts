...
export async function renderNePpPdf(form: FilledVsPpForm): Promise<Buffer> {
  const doc = new PDFDocument({
    size: "A4",
    margins: { top: 48, bottom: 48, left: 48, right: 48 },
    info: {
      Title: `Déclaration PP NE ${form.year} — ${form.company.name}`,
      Author: "Lexa",
      Subject: "Déclaration fiscale PP Canton de Neuchâtel",
    },
  });
  ...
  doc.fontSize(18).fillColor("#0b0b0f").text("Déclaration d'impôt PP Neuchâtel — 2026", {
    align: "left",
  });
  doc.fontSize(10).fillColor("#71717a").text("Canton de Neuchâtel — Service cantonal des contributions et de l'organisation (SCCO NE)");
  doc
    .fontSize(9)
    .text(
      `LCdir-NE (RSN 631.0), RGI-NE (RSN 631.01), LIFD — art. ${form.template.legal_reference.articles.join(", ")}`,
    );
  doc.moveDown(1);
  ...
  doc.text(`Canton : ${form.company.canton ?? "NE"}`);
  ...
  doc
    ...
    .fillColor("#991b1b")
    .text(
      "Information à titre indicatif — vérifiez avec votre fiduciaire et le Service cantonal des contributions et de l'organisation NE (SCCO).",
      48,
      760,
