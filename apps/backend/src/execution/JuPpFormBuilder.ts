...
// ── Constantes jurassiennes 2026 ─────────────────────────────────────────────
// Pilier 3a 2026 : avec LPP = 7'260 CHF, sans LPP = 36'288 CHF (identique fédéral)
const JU_PILIER_3A_WITH_LPP = 7260;
const JU_PILIER_3A_WITHOUT_LPP = 36288;

// Forfait frais professionnels JU — fallback 3% min 1'700 / max 3'400 CHF
// TODO : confirmer barème SCCJ (LICD-JU RSJU 641.11)
const JU_FRAIS_PRO_FORFAIT_MIN = 1700;
const JU_FRAIS_PRO_FORFAIT_MAX = 3400;
const JU_FRAIS_PRO_FORFAIT_PCT = 3;

export const JU_CONSTANTS = {
  JU_PILIER_3A_WITH_LPP,
  JU_PILIER_3A_WITHOUT_LPP,
  JU_FRAIS_PRO_FORFAIT_MIN,
  JU_FRAIS_PRO_FORFAIT_MAX,
  JU_FRAIS_PRO_FORFAIT_PCT,
} as const;

const TEMPLATE_FILE = "ju-declaration-pp-2026.yaml";
const __dirname = dirname(fileURLToPath(import.meta.url));

let cachedTemplate: VsPpFormTemplate | null = null;

async function loadJuPpTemplate(): Promise<VsPpFormTemplate> {
...
/**
 * Projection fiscale PP Jura.
 *
 * Clone de FrPpFormBuilder — adapté aux spécificités jurassiennes :
 ...
 * - Note : Moutier intégrée au JU depuis le 1er janvier 2021
...
function computeFraisProJu(
  ...
  const rawForfait = base * (JU_FRAIS_PRO_FORFAIT_PCT / 100);
  return round2(
    Math.min(
      Math.max(rawForfait, JU_FRAIS_PRO_FORFAIT_MIN),
      JU_FRAIS_PRO_FORFAIT_MAX,
    ),
  );
}

async function projectJuPp(params: {
  tenantId: string;
  year: number;
  ...
  const deductionFraisPro = computeFraisProJu(
...
export async function buildJuPpDeclaration(params: {
  tenantId: string;
  year: number;
  draft?: TaxpayerDraftState;
}): Promise<FilledVsPpForm> {
  const { tenantId, year, draft } = params;
  const template = await loadJuPpTemplate();
  const company: CompanyInfo = await getCompany(tenantId);
  const projection = await projectJuPp({
    tenantId,
    year,
