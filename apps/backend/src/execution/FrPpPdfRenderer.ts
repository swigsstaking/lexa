...
export async function renderFrPpPdf(form: FilledVsPpForm): Promise<Buffer> {
  const doc = new PDFDocument({
    size: "A4",
    margins: { top: 48, bottom: 48, left: 48, right: 48 },
    info: {
      Title: `Déclaration PP FR ${form.year} — ${form.company.name}`,
      Author: "Lexa",
      Subject: "Déclaration fiscale PP Canton de Fribourg",
    },
  });
  ...
  // ── Header ─────────────────────────────────────────
  doc.fontSize(18).fillColor("#0b0b0f").text("Déclaration d'impôt PP Fribourg — 2026", {
    align: "left",
  });
  doc.fontSize(10).fillColor("#71717a").text("Canton de Fribourg — Service cantonal des contributions (SCC FR)");
  doc
    .fontSize(9)
    .text(
      `LICD (BDLF 631.1), LIC (BDLF 632.1), ORD-FP (631.411), LIFD — art. ${form.template.legal_reference.articles.join(", ")}`,
    );
  doc.moveDown(1);

  // ── Disclaimer ────────────────────────────────────
  doc.rect(doc.x, doc.y, 500, 68).fillAndStroke("#fef2f2", "#ef4444");
  doc
    .fillColor("#991b1b")
    .fontSize(8)
  ...
  doc.text(`Canton : ${form.company.canton ?? "FR"}`);
  ...
  row(
    `Frais professionnels (forfait ${ref.frais_professionnels_forfait_pct}% min ${chf(ref.frais_professionnels_forfait_min_chf)} / max ${chf(ref.frais_professionnels_forfait_max_chf)} — TODO: confirmer SCC FR)`,
    p.deductionFraisPro,
  );
  row(
  ...
  // ── Footer ────────────────────────────────────────
  // Disclaimer obligatoire en bas de page — SCC FR
  doc
    .fontSize(7)
    .fillColor("#991b1b")
    .text(
      "Information à titre indicatif — vérifiez avec votre fiduciaire et le Service cantonal des contributions FR (SCC).",
      48,
      760,
