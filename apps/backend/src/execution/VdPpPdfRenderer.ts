...
export async function renderVdPpPdf(form: FilledVsPpForm): Promise<Buffer> {
  const doc = new PDFDocument({
    size: "A4",
    margins: { top: 48, bottom: 48, left: 48, right: 48 },
    info: {
      Title: `Déclaration PP VD ${form.year} — ${form.company.name}`,
      Author: "Lexa",
      Subject: "Déclaration fiscale PP Canton de Vaud",
    },
  });
  ...
  // ── Header ─────────────────────────────────────────
  doc.fontSize(18).fillColor("#0b0b0f").text("Déclaration d'impôt PP Vaud — 2026", {
    align: "left",
  });
  doc.fontSize(10).fillColor("#71717a").text("Canton de Vaud — Administration cantonale des impôts (ACI VD)");
  doc
    .fontSize(9)
    .text(
      `LI (BLV 642.11), LIPC (BLV 650.11), LIFD — art. ${form.template.legal_reference.articles.join(", ")}`,
    );
  doc.moveDown(1);
  ...
  doc.text(`Canton : ${form.company.canton ?? "VD"}`);
  ...
  // ── Footer ────────────────────────────────────────
  // Disclaimer obligatoire en bas de page — ACI VD
  doc
    .fontSize(7)
    .fillColor("#991b1b")
    .text(
      "Information à titre indicatif — vérifiez avec votre fiduciaire et l'Administration cantonale des impôts VD (ACI).",
      48,
      760,
