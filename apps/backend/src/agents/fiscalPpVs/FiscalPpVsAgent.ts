...
/**
 * FiscalPpVsAgent — Specialized agent for Valais personal income tax (PP).
 *
 * Uses:
 *   - lexa-fiscal-pp-vs Modelfile (qwen3.5:9b + SYSTEM prompt PP VS)
 *   - Qdrant prioritizing VS-Guide-PP / VS-Deductions / VS-Loi-fiscale,
 *     with federal fallback on LIFD / LHID / AFC-IFD-Circ.
 */
export class FiscalPpVsAgent {
  ...
  async ask(query: FiscalPpVsQuery): Promise<FiscalPpVsAnswer> {
    ...
    // 3. Re-rank boosting Valais-specific sources, then federal tax sources
    const rankedHits = this.rankVsTaxSources(hits).slice(0, 5);
    ...
    const prompt = `CONTEXTE FISCAL VS (extraits legaux):
${context}

QUESTION: ${enriched}

Reponds de maniere concise en citant les articles et les sections du Guide PP VS pertinents. Termine par l avertissement obligatoire.
...
REPONSE:`;
  ...
  private enrichQuestion(query: FiscalPpVsQuery): string {
    ...
    if (query.context?.commune) {
      parts.push(`Commune de domicile: ${query.context.commune} (Valais).`);
    }
  ...
  private rankVsTaxSources(hits: QdrantHit[]): QdrantHit[] {
    const tier = (law: string): number => {
      // 0 = sources VS prioritaires
      if (law === "VS-Guide-PP") return 0;
      if (law === "VS-Deductions") return 0;
      if (law === "VS-Loi-fiscale") return 1;
...
export const fiscalPpVsAgent = new FiscalPpVsAgent();
